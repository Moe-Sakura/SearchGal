...
lock = threading.Lock()
app = Flask(__name__)
CORS(app, origins=["https://searchgal.homes","https://searchgal.homes"])
...
if redis_password:
    try:
        redis_client = redis.Redis(host='175.178.127.190', port=6379, db=0, password=redis_password, decode_responses=True)
