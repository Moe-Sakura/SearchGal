...
def search_platform(platform, game, *args, **kwargs):
    ...
    try:
        ...
        try:
            ...
            if ("Search API 请求超时" in error) or ("Expecting value:" in error) or ("EOF occurred in violation of protocol" in error) or (("Search API 响应状态码为 " in error) and ("400" in error or "403" in error)):
